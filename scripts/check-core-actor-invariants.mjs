...
const tableDefinition = (text, tableName) => {
  const match = text.match(new RegExp(`CREATE TABLE IF NOT EXISTS ${tableName}\\s*\\([\\s\\S]*?\\n\\);`, 'i'));
  if (!match) throw new Error(`table definition not found: ${tableName}`);
  return match[0];
};

const actors = read('supabase/migrations/20260718100000_precreate_actors.sql');
const actorTable = tableDefinition(actors, 'actors');
