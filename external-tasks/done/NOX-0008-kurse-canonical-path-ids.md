...
### Schritt 1 — Migration 032
`supabase/migrations/032_kg_path_id.sql` (commit `972ac32`):
- `kg_path_id TEXT` Spalte auf `foundation_kurse` (nullable, idempotent)
- Mapping: `kurs_00_einheiten` + `kurs_01_prozentrechnung` → `PATH:SSF:MAT-FOUNDATIONS-0001`
- Im Supabase SQL-Editor auszuführen

### Schritt 2 — kurse/route.ts v1.3.0
`kg_path_id` wird jetzt in allen Kurs-Responses zurückgegeben.

### Schritt 3 — KursRenderer.tsx v1.2.0
`kg_path_id?` im Kurs-Interface ergänzt.

### Schritt 4 — SchoolOverlay.tsx v4.4.0
Neuer "Kurse"-Tab eingebunden:
- `KursListe`: lädt alle Kurse aus `/api/game/kurse`, zeigt Thema/Dauer/Punkte/Fortschritt
- `KursRenderer`: rendert Folien + Quiz wenn ein Kurs ausgewählt wird
- `KursRenderer` war bisher nicht verdrahtet — ist es jetzt

### Deployment-Mismatch
`KursRenderer` war implementiert aber nirgendwo eingebunden.
Mit SchoolOverlay v4.4.0 ist das behoben.
