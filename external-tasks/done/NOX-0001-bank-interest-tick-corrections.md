...
## Resolution
Fixed in `lib/game/tick.ts` v3.2.1 (commit `c406961`):
1. Kommentar korrigiert: "Sequenzieller Update + Ledger-Einträge; nicht transaktional"
2. Ledger-Fehler wird geloggt: `if (ledgerErr) console.error(...)`
3. `runBankInterestTick` ist jetzt `export async function`
