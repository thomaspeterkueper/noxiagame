...
## Final Resolution — 2026-07-15

**Item 1:** Bereits erledigt (commit `f1352e9`) — `TOPIC_DOMAIN` für calc-Tasks.

**Item 2:** Hardcodierte ECO-Module entfernt aus `SchoolOverlay.tsx` v4.4.1 (commit `01a61402`).
- Keine bestehenden Completion-Records in `academy_completions` für `LRN:SSF:ECO-L0-*`
- Module-Tab zeigt jetzt Hinweis auf KB-REQUEST-0003 (SSF-Umsetzung ausstehend)
- `knowledge/route.ts` v2.1.2 + `bank/route.ts` v0.4.2: zurück auf `academy_completions`
- `SchoolOverlay.tsx`: `player_learning_progress` → `academy_completions`
