create table if not exists public.core_sample_jobs (
  ...
  region_resource_id uuid references public.region_resources(id) on delete set null,
  result jsonb,
  created_at timestamptz not null default now()
);

create index if not exists core_sample_jobs_profile_location_idx
  on public.core_sample_jobs(profile_id, location_id, created_at desc);
create index if not exists core_sample_jobs_due_idx
  on public.core_sample_jobs(status, completes_at) where status='running';

grant select, insert, update on public.core_sample_jobs to service_role;
...
begin
  ...
  insert into public.core_sample_jobs(
    profile_id, location_id, latitude_deg, longitude_deg,
    energy_cost, component_cost, completes_at
  ) values (
    p_profile_id, p_location_id, p_latitude_deg, p_longitude_deg,
...
revoke all on function public.start_core_sample_job(uuid,uuid,double precision,double precision,integer,integer,integer) from public;
grant execute on function public.start_core_sample_job(uuid,uuid,double precision,double precision,integer,integer,integer) to service_role;
