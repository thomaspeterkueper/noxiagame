...
begin
  ...
  -- Earth vehicles are provisioned only into the canonical Earth location.
  if not exists (
    select 1
  ...
  -- Vehicle persistence owns living energy state. Seed the exact Engineering r1
  -- dispatchable battery budget so SurfaceMission can validate it authoritatively.
