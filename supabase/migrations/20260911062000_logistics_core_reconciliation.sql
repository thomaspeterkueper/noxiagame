-- NOXIA shared logistics core reconciliation
-- 2026-09-11
--
-- Production received the first logistics core through a manual rollout
-- (20260911060537). This migration makes the same contract reproducible from Git
-- for previews and fresh databases without replacing legacy location_resources or
-- ship_cargo storage. It is intentionally additive/idempotent.
...
create index if not exists logistics_reservations_active_idx
  on public.logistics_reservations(inventory_id, resource, direction)
  where status = 'active';
...
begin
  ...
  where id = p_inventory_id and active;
  if not found then
    raise exception 'NOXIA_INVENTORY_NOT_FOUND:%', p_inventory_id using errcode = 'P0001';
  end if;

  if v_inventory.storage_kind = 'native' then
    select amount into v_amount
    from public.logistics_inventory_items
    where inventory_id = p_inventory_id and resource = p_resource;
  elsif v_inventory.storage_kind = 'location_resources' then
    select stock into v_amount
    from public.location_resources
    where location_id = v_inventory.subject_id and resource = p_resource;
  elsif v_inventory.storage_kind = 'ship_cargo' then
    select amount into v_amount
    from public.ship_cargo
    where ship_id = v_inventory.subject_id and resource = p_resource;
  end if;
...
begin
  ...
  if v_inventory.storage_kind = 'native' then
    select coalesce(sum(amount), 0)::integer into v_amount
    from public.logistics_inventory_items where inventory_id = p_inventory_id;
  elsif v_inventory.storage_kind = 'location_resources' then
    select coalesce(sum(stock), 0)::integer into v_amount
    from public.location_resources where location_id = v_inventory.subject_id;
  elsif v_inventory.storage_kind = 'ship_cargo' then
    select coalesce(sum(amount), 0)::integer into v_amount
    from public.ship_cargo where ship_id = v_inventory.subject_id;
  end if;
...
begin
  ...
  if v_next < 0 then
    raise exception 'NOXIA_INVENTORY_STOCK_INSUFFICIENT:%:%', p_resource, v_current using errcode = 'P0001';
  end if;
  ...
  if v_inventory.storage_kind = 'native' then
    ...
    on conflict (inventory_id, resource)
...
begin
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'available', greatest(0,
      public.noxia_inventory_amount(p_inventory_id, r.type)
      - public.noxia_inventory_reserved(p_inventory_id, r.type, 'outbound')
    )
  ) order by r.type::text), '[]'::jsonb)
  into v_items
  from public.resources r;

  v_capacity := public.noxia_inventory_capacity(p_inventory_id);
  return to_jsonb(v_inventory) || jsonb_build_object(
    'capacity', v_capacity,
    'totalAmount', public.noxia_inventory_total_amount(p_inventory_id),
    'items', v_items
  );
end;
$$;
...
declare
  ...
  v_result jsonb;
begin
  if p_command_id is null or p_actor_profile_id is null
     or p_source_inventory_id is null or p_target_inventory_id is null
     or p_amount is null or p_amount <= 0 then
    raise exception 'NOXIA_CARGO_TRANSFER_INVALID_ARGUMENT' using errcode = 'P0001';
  end if;
  if p_source_inventory_id = p_target_inventory_id then
    raise exception 'NOXIA_CARGO_TRANSFER_SAME_INVENTORY' using errcode = 'P0001';
  end if;
  ...
  perform id from public.logistics_inventories
  where id in (p_source_inventory_id, p_target_inventory_id)
  ...
  if v_source.owner_profile_id is distinct from p_actor_profile_id and not v_source.public_withdraw then
    raise exception 'NOXIA_INVENTORY_SOURCE_FORBIDDEN' using errcode = 'P0001';
  end if;
  if v_target.owner_profile_id is distinct from p_actor_profile_id and not v_target.public_deposit then
    raise exception 'NOXIA_INVENTORY_TARGET_FORBIDDEN' using errcode = 'P0001';
  end if;
  ...
  if v_source_amount - v_source_reserved < p_amount then
    raise exception 'NOXIA_INVENTORY_AVAILABLE_INSUFFICIENT:%:%', p_resource,
      greatest(0, v_source_amount - v_source_reserved) using errcode = 'P0001';
  end if;
  ...
  if v_target_capacity is not null then
    ...
    select coalesce(sum(amount),0)::integer into v_target_reserved
    from public.logistics_reservations
    where inventory_id = p_target_inventory_id and direction = 'inbound' and status = 'active';
    if v_target_total + v_target_reserved + p_amount > v_target_capacity then
      raise exception 'NOXIA_INVENTORY_CAPACITY_INSUFFICIENT:%:%', v_target_capacity,
        v_target_total + v_target_reserved using errcode = 'P0001';
    end if;
  end if;
  ...
  v_result := jsonb_build_object(
    ...
    'sourceInventoryId', p_source_inventory_id,
    'targetInventoryId', p_target_inventory_id,
    'resource', p_resource,
    'amount', p_amount,
    'sourceAmount', v_source_after,
    'targetAmount', v_target_after,
    'idempotent', false
  );
  insert into public.cargo_transfer_commands(
    command_id, actor_profile_id, source_inventory_id, target_inventory_id, resource, amount, result
  ) values (
    p_command_id, p_actor_profile_id, p_source_inventory_id, p_target_inventory_id, p_resource, p_amount, v_result
  );
  return v_result;
end;
$$;
...
begin
  if p_command_id is null or p_actor_profile_id is null
     or p_source_inventory_id is null or p_destination_inventory_id is null
     or p_amount is null or p_amount <= 0 then
    raise exception 'NOXIA_TRANSPORT_JOB_INVALID_ARGUMENT' using errcode='P0001';
  end if;

  select * into v_existing from public.transport_jobs where command_id = p_command_id;
  if found then
    if v_existing.actor_profile_id is distinct from p_actor_profile_id then
  ...
  perform id from public.logistics_inventories
  where id in (p_source_inventory_id, p_destination_inventory_id, p_vehicle_inventory_id)
  order by id for update;
  select * into v_source from public.logistics_inventories where id = p_source_inventory_id and active;
  ...
  if p_domain = 'surface' and coalesce((p_route_snapshot->>'passable')::boolean, false) is not true then
    raise exception 'NOXIA_TRANSPORT_ROUTE_NOT_PASSABLE' using errcode='P0001';
  end if;

  v_available := public.noxia_inventory_amount(p_source_inventory_id, p_resource)
    - public.noxia_inventory_reserved(p_source_inventory_id, p_resource, 'outbound');
  if v_available < p_amount then
    raise exception 'NOXIA_INVENTORY_AVAILABLE_INSUFFICIENT:%:%', p_resource,
      greatest(0, v_available) using errcode='P0001';
  end if;
  ...
  if v_capacity is not null then
    ...
    select coalesce(sum(amount),0)::integer into v_inbound
    from public.logistics_reservations
    where inventory_id = p_destination_inventory_id and direction = 'inbound' and status = 'active';
    if v_total + v_inbound + p_amount > v_capacity then
      raise exception 'NOXIA_INVENTORY_CAPACITY_INSUFFICIENT:%:%', v_capacity,
        v_total + v_inbound using errcode='P0001';
    end if;
  end if;

  if p_vehicle_inventory_id is not null then
    select * into v_vehicle from public.logistics_inventories
    where id = p_vehicle_inventory_id and active;
    if not found or v_vehicle.inventory_kind <> 'vehicle' then
      raise exception 'NOXIA_TRANSPORT_VEHICLE_INVALID' using errcode='P0001';
    end if;
    if v_vehicle.owner_profile_id is distinct from p_actor_profile_id then
      raise exception 'NOXIA_TRANSPORT_VEHICLE_FORBIDDEN' using errcode='P0001';
    end if;
    v_capacity := public.noxia_inventory_capacity(p_vehicle_inventory_id);
    if v_capacity is not null
       and public.noxia_inventory_total_amount(p_vehicle_inventory_id) + p_amount > v_capacity then
      raise exception 'NOXIA_TRANSPORT_VEHICLE_CAPACITY_INSUFFICIENT' using errcode='P0001';
    end if;
  end if;

  begin
    insert into public.transport_jobs(
      id, command_id, actor_profile_id, location_id, domain,
      source_inventory_id, destination_inventory_id,
      vehicle_inventory_id, vehicle_role, resource, amount, status, route_snapshot
    ) values (
      v_job_id, p_command_id, p_actor_profile_id, p_location_id,
      coalesce(nullif(p_domain,''), 'surface'),
      p_source_inventory_id, p_destination_inventory_id,
      p_vehicle_inventory_id, p_vehicle_role, p_resource, p_amount, 'reserved',
      coalesce(p_route_snapshot, '{}'::jsonb)
    );
  ...
  insert into public.logistics_reservations(
    inventory_id, actor_profile_id, resource, amount, direction, purpose_type, purpose_id
  ) values
  ...
  return (select to_jsonb(j) from public.transport_jobs j where j.id = v_job_id)
...
begin
  ...
  if v_job.status = 'in_transit' then
    return to_jsonb(v_job) || jsonb_build_object('idempotent', true);
  end if;
  if v_job.status <> 'reserved' then
    raise exception 'NOXIA_TRANSPORT_JOB_STATE_INVALID:%', v_job.status using errcode='P0001';
  end if;
  if v_job.vehicle_inventory_id is null then
    raise exception 'NOXIA_TRANSPORT_VEHICLE_REQUIRED' using errcode='P0001';
  end if;
  if v_job.domain = 'surface'
     and coalesce((v_job.route_snapshot->>'passable')::boolean, false) is not true then
    raise exception 'NOXIA_TRANSPORT_ROUTE_NOT_PASSABLE' using errcode='P0001';
  end if;
  if public.noxia_inventory_amount(v_job.source_inventory_id, v_job.resource) < v_job.amount then
    raise exception 'NOXIA_INVENTORY_STOCK_INSUFFICIENT' using errcode='P0001';
  end if;

  v_capacity := public.noxia_inventory_capacity(v_job.vehicle_inventory_id);
  if v_capacity is not null
     and public.noxia_inventory_total_amount(v_job.vehicle_inventory_id) + v_job.amount > v_capacity then
    raise exception 'NOXIA_TRANSPORT_VEHICLE_CAPACITY_INSUFFICIENT' using errcode='P0001';
  end if;
  ...
  where purpose_type = 'transport_job' and purpose_id = v_job.id
    and direction = 'outbound' and status = 'active';

  if coalesce(v_job.route_snapshot->>'etaSeconds','') ~ '^[0-9]+$' then
    v_eta := (v_job.route_snapshot->>'etaSeconds')::integer;
  end if;

  update public.transport_jobs
  set status = 'in_transit',
      started_at = coalesce(started_at, now()),
      arrives_at = case when v_eta is not null and v_eta > 0
        then now() + make_interval(secs => v_eta) else arrives_at end,
      updated_at = now()
  where id = v_job.id
...
begin
  ...
  if v_job.status in ('arrived','unloading','completed') then
    return to_jsonb(v_job) || jsonb_build_object('idempotent', true);
  end if;
  if v_job.status <> 'in_transit' then
    raise exception 'NOXIA_TRANSPORT_JOB_STATE_INVALID:%', v_job.status using errcode='P0001';
  end if;
  if v_job.arrives_at is not null and v_job.arrives_at > now() then
    raise exception 'NOXIA_TRANSPORT_NOT_ARRIVED:%',
      extract(epoch from (v_job.arrives_at - now()))::integer using errcode='P0001';
  end if;
...
begin
  ...
  if public.noxia_inventory_amount(v_job.vehicle_inventory_id, v_job.resource) < v_job.amount then
    raise exception 'NOXIA_TRANSPORT_VEHICLE_CARGO_MISSING' using errcode='P0001';
  end if;

  update public.logistics_reservations
  set status = 'consumed', settled_at = now()
  where purpose_type = 'transport_job' and purpose_id = v_job.id
    and direction = 'inbound' and status = 'active';
  ...
  if v_capacity is not null then
    ...
    select coalesce(sum(amount),0)::integer into v_other_inbound
    from public.logistics_reservations
    where inventory_id = v_job.destination_inventory_id
      and direction = 'inbound' and status = 'active';
    if v_total + v_other_inbound + v_job.amount > v_capacity then
      raise exception 'NOXIA_INVENTORY_CAPACITY_INSUFFICIENT' using errcode='P0001';
    end if;
  end if;
...
begin
  ...
  if v_job.status <> 'reserved' then
    raise exception 'NOXIA_TRANSPORT_CANCEL_REQUIRES_RESERVED:%', v_job.status using errcode='P0001';
  end if;

  update public.logistics_reservations
  set status = 'released', settled_at = now()
  where purpose_type = 'transport_job' and purpose_id = v_job.id and status = 'active';
  update public.transport_jobs
...
-- The shared location inventory is a compatibility adapter over the current
-- location_resources store. This does not move or duplicate legacy stock.
insert into public.logistics_inventories(
  owner_profile_id, location_id, inventory_kind, storage_kind,
  subject_type, subject_id, label, capacity, public_deposit, public_withdraw, metadata
)
select
  null, l.id, 'location', 'location_resources', 'location', l.id,
  l.name || ' · Standortbestand', null, false, false,
  jsonb_build_object('legacyAggregate', true)
from public.locations l
where exists (select 1 from public.location_resources lr where lr.location_id = l.id)
...
-- Existing ships keep ship_cargo as their physical cargo store; the inventory
-- record only gives the common logistics core a stable address for it.
insert into public.logistics_inventories(
  owner_profile_id, location_id, inventory_kind, storage_kind,
  subject_type, subject_id, label, capacity, public_deposit, public_withdraw, metadata
)
select
  s.profile_id, l.id, 'vehicle', 'ship_cargo', 'ship', s.id,
  coalesce(nullif(s.name,''), 'Schiff'), s.cargo_max, false, false,
...
-- First playable Surface Logistics slice: Moon/Shackleton. No historic
-- location_resources stock is copied into these native inventories.
insert into public.logistics_inventories(
  owner_profile_id, location_id, inventory_kind, storage_kind,
  subject_type, subject_id, label, capacity, public_deposit, public_withdraw, metadata
)
select
  te.profile_id, te.location_id, 'facility', 'native', 'tile_entity', te.id,
  'Mine · Minenpuffer', null, false, false,
  jsonb_build_object('buildingType','mine','ownerClass',coalesce(te.owner_class,'UNKNOWN'))
from public.tile_entities te
join public.locations l on l.id = te.location_id
where l.slug = 'moon' and te.entity_type = 'building' and te.entity_id = 'mine'
on conflict (storage_kind, subject_type, subject_id) do update
set owner_profile_id = excluded.owner_profile_id,
    location_id = excluded.location_id,
...
select
  te.profile_id, te.location_id, 'depot', 'native', 'tile_entity', te.id,
  'Warenhaus · Logistik-Hub', null, true, true,
  jsonb_build_object('buildingType','warehouse','ownerClass',coalesce(te.owner_class,'UNKNOWN'))
from public.tile_entities te
join public.locations l on l.id = te.location_id
where l.slug = 'moon' and te.entity_type = 'building' and te.entity_id = 'warehouse'
on conflict (storage_kind, subject_type, subject_id) do update
set owner_profile_id = excluded.owner_profile_id,
    location_id = excluded.location_id,
    inventory_kind = excluded.inventory_kind,
...
select
  te.profile_id, te.location_id, 'surface_port', 'native', 'tile_entity', te.id,
  'Shuttle-Port-Lager', null, true, true,
  jsonb_build_object('buildingType','landing_pad','ownerClass',coalesce(te.owner_class,'UNKNOWN'))
from public.tile_entities te
join public.locations l on l.id = te.location_id
where l.slug = 'moon' and te.entity_type = 'building' and te.entity_id = 'landing_pad'
on conflict (storage_kind, subject_type, subject_id) do update
set owner_profile_id = excluded.owner_profile_id,
    location_id = excluded.location_id,
    inventory_kind = excluded.inventory_kind,
...
comment on table public.logistics_inventories is
  'Addressable logistics inventory adapters and native stores shared across surface, orbital and inter-node logistics.';
comment on table public.transport_jobs is
  'Persistent logistics jobs. Route policy is supplied by world domains; inventory and state transitions are owned by Core.';
comment on function public.noxia_transfer_cargo(uuid, uuid, uuid, uuid, public.resource_type, integer) is
  'Atomic, idempotent inventory-to-inventory cargo handover.';
