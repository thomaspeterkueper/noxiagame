...
begin
  if tg_op = 'INSERT' then
    ...
    v_effects := jsonb_build_array(
      jsonb_build_object(
        'type', 'build_status',
        'buildable_id', new.buildable_id,
        'status', new.status,
        'tile_level', new.tile_level,
        'tile_row', new.tile_row,
        'tile_col', new.tile_col,
  ...
  insert into public.events (
    ...
    new.location_id,
    v_effects,
    jsonb_build_object('source', 'player_builds_trigger'),
    now()
  );
...
after insert or update of status, completes_at
...
begin
  ...
  if tg_op = 'INSERT' then
    v_effects := jsonb_build_array(
      jsonb_build_object('type', 'entity_state', 'to', v_properties)
    );
  else
    v_effects := jsonb_build_array(
      jsonb_build_object(
        ...
        'from', jsonb_build_object(
          ...
          'tile_level', old.tile_level,
          'tile_row', old.tile_row,
          'tile_col', old.tile_col,
          'owner_class', old.owner_class,
          'owner_id', old.owner_id,
          'occupant_id', old.occupant_id,
  ...
  insert into public.events (
    ...
    'tile_entity',
    new.id,
    coalesce(new.profile_id, new.actor_id),
    new.location_id,
    v_effects,
    jsonb_build_object('source', 'tile_entities_trigger'),
    now()
  ) returning id into v_event_id;

  update public.entity_states
  set valid_to = now()
  where subject_type = 'tile_entity'
...
comment on function public.noxia_record_player_build_event() is
  'Authoritative audit/event projection for player_builds lifecycle changes.';
comment on function public.noxia_record_tile_entity_state() is
  'Projects tile_entities mutations into generalized events and temporal entity_states.';
