...
-- Databases where the table was created by the re-applied corrected
-- 2026-08-31 foundation migration (or its pre-PR form) may lack the
-- canonical projection columns; converge them like entity_states below.
...
-- Databases where the pre-PR 20260831164500 wire migration is recorded keep its
-- old trigger functions, whose bodies insert into legacy public.events
-- (event_type/subject_type/... columns) and fail at runtime. Re-establish the
-- corrected functions and triggers here so recorded databases converge to the
-- same runtime shape as fresh databases; the repaired 20260831164500 file
-- itself is skipped wherever its old version is recorded.
