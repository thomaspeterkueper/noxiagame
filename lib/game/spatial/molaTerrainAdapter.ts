...
/**
 * Source-specific adapter for MGS/MOLA global topography rasters.
 *
 * MOLA topography is interpreted as elevation relative to the source vertical
 * reference. Raster decoding stays injected so the simulation domain remains
 * independent of one GeoTIFF/PDS decoder or storage backend.
 */
export class MolaTerrainAdapter implements TerrainRasterAdapter {
  ...
  supports(dataset: TerrainDatasetDescriptor) {
    return dataset.id === 'mars_mgs_mola_megdr_463m'
  }
  ...
  async sampleAtPlanetary(
    ...
    const scale = metadataNumber(dataset, 'stored_scale', 1)
...
export const MARS_MOLA_DATASET: TerrainDatasetDescriptor = {
  id: 'mars_mgs_mola_megdr_463m',
  body: 'mars',
  provider: 'NASA PDS / USGS Astrogeology',
  datasetName: 'Mars Global Surveyor MOLA MEGDR Topography',
  datasetVersion: '2.0',
  datasetKind: 'dem',
  resolutionM: 463,
  horizontalReference: 'IAU_MARS_SIMPLE_CYLINDRICAL',
