...
export interface FindInteriorRouteOptions {
  /** Additional access-policy restriction; never overrides operational state. */
...
export function findInteriorRoute(
  ...
  while (queue.length) {
    ...
    for (const portal of template.portals) {
      ...
      const operationallyTraversable = isPortalTraversable(instance, portal.id)
      const policyPermitted = options.canUsePortal ? options.canUsePortal(portal.id) : true
      if (!operationallyTraversable || !policyPermitted) continue
