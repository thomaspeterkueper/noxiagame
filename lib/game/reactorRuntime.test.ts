...
const allOnline = buildReactorRuntimeSummary({
  assets,
  currentStates: assets.map(asset => state(asset.tileEntityId, 'online', 1)),
})
...
const halfDerated = buildReactorRuntimeSummary({
  assets,
  currentStates: assets.map((asset, index) => index === 5
    ? state(asset.tileEntityId, 'derated', 0.5, 'thermal_margin')
    : state(asset.tileEntityId, 'online', 1)),
})
...
const oneOffline = buildReactorRuntimeSummary({
  assets,
  currentStates: assets.map((asset, index) => index === 0
    ? state(asset.tileEntityId, 'offline', 0, 'maintenance')
    : state(asset.tileEntityId, 'online', 1)),
})
...
const missing = buildReactorRuntimeSummary({
  assets,
  currentStates: assets.slice(0, 5).map(asset => state(asset.tileEntityId, 'online', 1)),
})
...
const unknown = buildReactorRuntimeSummary({
  assets,
  currentStates: assets.map((asset, index) => index === 2
    ? state(asset.tileEntityId, 'unknown', null)
    : state(asset.tileEntityId, 'online', 1)),
})
...
const invalid = buildReactorRuntimeSummary({
  assets,
  currentStates: assets.map((asset, index) => index === 1
    ? state(asset.tileEntityId, 'online', 0.5)
    : state(asset.tileEntityId, 'online', 1)),
})
...
const duplicate = buildReactorRuntimeSummary({
  assets,
  currentStates: [
    ...assets.map(asset => state(asset.tileEntityId, 'online', 1)),
    state(assets[0].tileEntityId, 'offline', 0),
  ],
})
...
const duplicateAuthority = buildReactorRuntimeSummary({
  assets,
  currentStates: assets.map((asset, index) => index === 3
    ? {
        ...state(asset.tileEntityId, 'online', 1),
        properties: {
          schemaVersion: 1,
