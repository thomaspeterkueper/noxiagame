...
export function buildReactorRuntimeSummary(input: {
  ...
  const reactors: ReactorRuntimeObservation[] = input.assets.map(asset => {
    ...
    if (rows.length === 0) {
      return { tileEntityId: asset.tileEntityId, status: 'missing', mode: null, availabilityFactor: null, availablePowerMw: null, reasonCode: null, sourceRef: null, issue: 'No authoritative reactor_runtime state exists for this reactor.' }
    }
    if (rows.length !== 1) {
      return { tileEntityId: asset.tileEntityId, status: 'invalid', mode: null, availabilityFactor: null, availablePowerMw: null, reasonCode: null, sourceRef: null, issue: 'Multiple current reactor_runtime states exist for one reactor.' }
    }

    const row = rows[0]
    const parsed = runtimeProperties(row.properties)
    if (!parsed) {
      return { tileEntityId: asset.tileEntityId, status: 'invalid', mode: null, availabilityFactor: null, availablePowerMw: null, reasonCode: null, sourceRef: null, issue: 'Stored reactor_runtime properties violate the E2a contract.' }
    }
    ...
    return {
      ...
      availablePowerMw: known && nominalValid ? roundMw(nominalPowerMw * parsed.availabilityFactor!) : null,
      reasonCode: parsed.reasonCode,
      sourceRef: row.canonicalStateId ? `core:entity_states:${row.canonicalStateId}` : row.sourceEvent ? `core:simulation_events:${row.sourceEvent}` : `core:entity_states:reactor_runtime:${asset.tileEntityId}`,
      issue: !nominalValid ? 'Canonical nominal reactor power is missing or invalid.' : parsed.mode === 'unknown' ? 'Runtime mode is explicitly unknown.' : null,
    }
  })

  const observedStates = reactors.filter(reactor => reactor.status === 'observed').length
  const knownStates = reactors.filter(reactor => reactor.status === 'observed' && reactor.mode !== 'unknown' && reactor.availablePowerMw !== null).length
  const unresolvedStates = reactors.length - knownStates

  let availablePowerMw: AvailableReactorPowerObservation
  if (input.assets.length === 0) {
    availablePowerMw = { status: 'unresolved', reason: 'No live reactor assets are available for runtime observation.' }
  } else if (unresolvedStates > 0) {
    availablePowerMw = { status: 'unresolved', reason: `${unresolvedStates} of ${reactors.length} reactor runtime states are missing, unknown or invalid; missing state is never interpreted as zero or full power.` }
  } else {
    availablePowerMw = { status: 'observed', valueMw: roundMw(reactors.reduce((sum, reactor) => sum + Number(reactor.availablePowerMw ?? 0), 0)), sourceRef: 'core:entity_states:reactor_runtime+seed:tharsisHubSeed:nominal-reactor-power' }
  }

  return { totalReactors: reactors.length, observedStates, knownStates, unresolvedStates, reactors, availablePowerMw }
}
