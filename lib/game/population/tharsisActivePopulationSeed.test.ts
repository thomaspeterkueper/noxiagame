...
const migration = readFileSync(resolve(here, '../../../supabase/migrations/20260831103000_tharsis_active_population_seed.sql'), 'utf8')

if (THARSIS_HUB_POPULATION !== 497) throw new Error('canonical Tharsis aggregate population changed')
if (!migration.includes('FOR g IN 1..36 LOOP')) throw new Error('active cohort must contain exactly 36 generated people')
...
for (const id of requiredWorkObjects) {
  ...
  if (!migration.includes(`work_seed := '${id}'`)) throw new Error(`migration does not bind role to ${id}`)
}
