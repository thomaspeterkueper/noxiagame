...
export default function EarthRegionPreview(){
  ...
  useEffect(()=>{fetch('/api/earth/region?radiusKm=3').then(r=>r.json()).then(setData).catch(e=>setData({ok:false,error:String(e)}));fetch('/api/earth/spaceport-candidates?radiusKm=3').then(r=>r.json()).then(setCandidateData).catch(e=>setCandidateData({ok:false,error:String(e)}));void loadSpatial()},[])
  ...
  useEffect(()=>{if(!data?.bounds||!data.region?.origin)return;const origin=data.region.origin,b=data.bounds,nw=geoToLocalMeters({lat:b.north,lon:b.west},origin),se=geoToLocalMeters({lat:b.south,lon:b.east},origin);const q=new URLSearchParams({minXM:String(Math.min(nw.eastM,se.eastM)),minYM:String(Math.min(nw.northM,se.northM)),maxXM:String(Math.max(nw.eastM,se.eastM)),maxYM:String(Math.max(nw.northM,se.northM)),maxBuildableSlopeDeg:'5',maxRestrictedSlopeDeg:'12',resolutionM:'120'});fetch(`/api/earth/buildability?${q}`).then(r=>r.json()).then(setTerrain).catch(e=>setTerrain({ok:false,error:String(e)}))},[data])
  ...
  function featureVisible(type:string){if(type==='water'||type==='waterway')return layers.water;if(type==='forest'||type==='farmland')return layers.landuse;if(type==='road'||type==='rail'||type==='industrial'||type==='public'||type==='building')return layers.infrastructure;return true}
  ...
  function focusMapPoint(mapX:number,mapY:number,label:string){if(!mapMetrics||buildTypeId)return;const nextZoom=clamp(mapMetrics.widthM/BUILD_PLAN_VISIBLE_WIDTH_M,.7,128);setCamera(nextZoom,mapX,mapY);setFocusLabel(label)}
  function resetOverview(){setZoom(1);setOffset({x:0,y:0});setFocusLabel(null)}
  function zoomAroundCenter(direction:1|-1){const factor=direction>0?1.15:.87,nextZoom=clamp(zoom*factor,.7,128),centerX=(500-offset.x)/zoom,centerY=(500-offset.y)/zoom;setCamera(nextZoom,centerX,centerY);if(nextZoom<2)setFocusLabel(null)}

  function pointerToPreview(e:React.PointerEvent<HTMLElement>){if(!projection||!data?.region?.origin||!mapGroupRef.current)return null;const svg=mapGroupRef.current.ownerSVGElement,ctm=mapGroupRef.current.getScreenCTM();if(!svg||!ctm)return null;const point=svg.createSVGPoint();point.x=e.clientX;point.y=e.clientY;const local=point.matrixTransform(ctm.inverse()),geo={lon:projection.lon(local.x),lat:projection.lat(local.y)},metric=geoToLocalMeters(geo,data.region.origin);return{mapX:local.x,mapY:local.y,xM:metric.eastM,yM:metric.northM}}

  async function placeBuilding(){if(!selectedBuilding||!preview||placing)return;setPlacing(true);setBuildMessage('Bauauftrag wird geprüft …');try{const token=await getToken();if(!token)throw new Error('Nicht angemeldet');const response=await fetch('/api/game/build/spatial',{method:'POST',headers:{Authorization:`Bearer ${token}`,'Content-Type':'application/json'},body:JSON.stringify({buildableId:selectedBuilding.id,location:'earth',xM:preview.xM,yM:preview.yM})});const json=await response.json();if(!response.ok)throw new Error(json.error??'Bauauftrag fehlgeschlagen');setBuildMessage(`${selectedBuilding.name}: Bauauftrag angelegt`);await loadSpatial()}catch(error){setBuildMessage(error instanceof Error?error.message:String(error))}finally{setPlacing(false)}}
    ...
    <div className={`earth-map ${buildTypeId?'building':''}`}
      ...
      <svg viewBox="0 0 1000 1000" preserveAspectRatio="xMidYMid slice">
        ...
        <g ref={mapGroupRef} transform={`translate(${offset.x} ${offset.y}) scale(${zoom})`}>
          ...
          {layers.noxia&&placed.map(b=>{const visual=b.visual,spriteScale=visual?.mapScale??1.7,spriteW=Math.max(b.widthSvg*spriteScale,22/zoom),spriteH=Math.max(Math.max(b.depthSvg,b.widthSvg*.72)*spriteScale,18/zoom);return <g key={`noxia-${b.id}`} transform={`translate(${b.mapX} ${b.mapY}) rotate(${b.rotation??0})`}><ellipse cx={0} cy={b.depthSvg*.18} rx={Math.max(b.widthSvg*.58,5/zoom)} ry={Math.max(b.depthSvg*.32,2.4/zoom)} fill="#14271e" opacity={b.pending?.16:.24}/><rect x={-b.widthSvg/2} y={-b.depthSvg/2} width={Math.max(b.widthSvg,2/zoom)} height={Math.max(b.depthSvg,2/zoom)} rx={1/zoom} fill={b.pending?'#d9a63d':'#eaf1df'} fillOpacity={b.pending?.18:.07} stroke={b.pending?'#9b741f':'#284f54'} strokeOpacity={b.pending?.9:.55} strokeWidth={1.1/zoom} strokeDasharray={b.pending?`${3/zoom} ${2/zoom}`:undefined}/>{visual?.mapAsset?<image href={visual.mapAsset} x={-spriteW/2} y={-spriteH*.72} width={spriteW} height={spriteH} preserveAspectRatio="xMidYMid meet" opacity={b.pending?.58:1}/>:<rect x={-b.widthSvg/2} y={-b.depthSvg/2} width={Math.max(b.widthSvg,2/zoom)} height={Math.max(b.depthSvg,2/zoom)} rx={1/zoom} fill={b.pending?'#d9a63d':'#1f5967'} fillOpacity={b.pending?.7:.9}/>} {zoom>=3&&<text x={0} y={-spriteH*.58-5/zoom} textAnchor="middle" fontSize={9/zoom} fontWeight="800" fill="#17313c" paintOrder="stroke" stroke="#f5f2e8" strokeWidth={2/zoom}>{b.name}{b.pending?' · Bau':''}</text>}</g>})}
      ...
      <div className="earth-help">{buildTypeId?'Bauposition wählen · Esc: abbrechen':'Stadt/Punkt klicken: Bauplan · Mausrad: Zoom · Ziehen: Karte'}</div>
    </div>
