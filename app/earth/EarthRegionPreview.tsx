...
export default function EarthRegionPreview(){
  ...
  const loadSpatial=async()=>{
    ...
    if(!token){setSpatial({error:'Nicht angemeldet'});return}
    const response=await fetch('/api/game/build/spatial?location=earth',{headers:{Authorization:`Bearer ${token}`}})
    setSpatial(await response.json())
  }

  useEffect(()=>{
    fetch('/api/earth/region?radiusKm=3').then(r=>r.json()).then(setData).catch(e=>setData({ok:false,error:String(e)}))
  ...
  useEffect(()=>{
    const cancel=(e:KeyboardEvent)=>{if(e.key==='Escape'){setBuildTypeId(null);setPreview(null);setBuildMessage(null)}}
  ...
  const projection=useMemo(()=>{
    ...
    const b=data.bounds
    return{
      x:(lon:number)=>((lon-b.west)/(b.east-b.west))*1000,
      y:(lat:number)=>((b.north-lat)/(b.north-b.south))*1000,
      lon:(x:number)=>b.west+x/1000*(b.east-b.west),
      lat:(y:number)=>b.north-y/1000*(b.north-b.south),
    }
  },[data])

  const mapMetrics=useMemo(()=>{
    if(!data?.bounds)return null
    const b=data.bounds,midLat=(b.south+b.north)/2,midLon=(b.west+b.east)/2
    return{
      widthM:distanceMeters({lat:midLat,lon:b.west},{lat:midLat,lon:b.east}),
      heightM:distanceMeters({lat:b.south,lon:midLon},{lat:b.north,lon:midLon}),
    }
  },[data])

  useEffect(()=>{
    if(!data?.bounds||!data.region?.origin)return
    const origin=data.region.origin,b=data.bounds
    const nw=geoToLocalMeters({lat:b.north,lon:b.west},origin),se=geoToLocalMeters({lat:b.south,lon:b.east},origin)
    const q=new URLSearchParams({
      minXM:String(Math.min(nw.eastM,se.eastM)),minYM:String(Math.min(nw.northM,se.northM)),
      maxXM:String(Math.max(nw.eastM,se.eastM)),maxYM:String(Math.max(nw.northM,se.northM)),
      maxBuildableSlopeDeg:'5',maxRestrictedSlopeDeg:'12',resolutionM:'120',
    })
    fetch(`/api/earth/buildability?${q}`).then(r=>r.json()).then(setTerrain).catch(e=>setTerrain({ok:false,error:String(e)}))
  },[data])

  const projected=useMemo(()=>{
    if(!projection||!data?.features)return[]
    return[...data.features].sort((a,b)=>layerOrder.indexOf(a.featureType)-layerOrder.indexOf(b.featureType)).map(f=>
      f.geometry.kind==='point'
        ? {...f,p:[projection.x(f.geometry.coordinates.lon),projection.y(f.geometry.coordinates.lat)]as[number,number]}
        : {...f,d:f.geometry.coordinates.map((p,i)=>`${i?'L':'M'}${projection.x(p.lon).toFixed(2)} ${projection.y(p.lat).toFixed(2)}`).join(' ')+(f.geometry.kind==='polygon'?' Z':'')}
    )
  },[data,projection])
  ...
  const scale=useMemo(()=>{
    ...
    const visibleWidthM=mapMetrics.widthM/zoom,targetM=visibleWidthM/5
    const options=[2,5,10,20,50,100,200,500,1000,2000,5000,10000]
    const meters=options.reduce((best,n)=>Math.abs(n-targetM)<Math.abs(best-targetM)?n:best,options[0])
    return{meters,pixels:meters/mapMetrics.widthM*1000*zoom}
  },[mapMetrics,zoom])

  const terrainOverlay=useMemo(()=>{
    if(!terrain?.ok||!terrain.cells||!projection||!data?.region?.origin||!mapMetrics)return[]
    const cells=terrain.cells,byKey=new Map(cells.map(c=>[`${c.row}:${c.col}`,c]))
    const stepX=(terrain.sourceResolutionM??120)/mapMetrics.widthM*1000,stepY=(terrain.sourceResolutionM??120)/mapMetrics.heightM*1000
    return cells.map(c=>{
      const geo=localMetersToGeo({eastM:c.xM,northM:c.yM},data.region!.origin)
      const x=projection.x(geo.lon),y=projection.y(geo.lat)
      const west=byKey.get(`${c.row}:${c.col-1}`),east=byKey.get(`${c.row}:${c.col+1}`),north=byKey.get(`${c.row-1}:${c.col}`),south=byKey.get(`${c.row+1}:${c.col}`)
      const dx=(east?.elevationM??c.elevationM)-(west?.elevationM??c.elevationM),dy=(south?.elevationM??c.elevationM)-(north?.elevationM??c.elevationM)
      const light=clamp(.52+(-dx+dy)*.012,.18,.82)
      return{
        ...c,x,y,w:stepX,h:stepY,
        shade:light<.5?'#203026':'#fff7da',shadeOpacity:Math.abs(light-.5)*.42,
        buildFill:c.state==='buildable'?'#4ecb71':c.state==='restricted'?'#e6bc46':c.state==='invalid'?'#d85757':'#78818a',
        slopeOpacity:clamp((c.slopeDeg??0)/22,0,.55),
      }
    })
  },[terrain,projection,data,mapMetrics])
  ...
  const placed=useMemo(()=>{
    ...
    const rows=[
      ...(spatial?.entities??[]).map(e=>({id:e.id,typeId:e.entity_id,xM:e.x_m,yM:e.y_m,rotation:e.rotation_deg,widthM:e.footprint_width_m,depthM:e.footprint_depth_m,pending:false})),
      ...(spatial?.builds??[]).map(b=>({id:b.id,typeId:b.buildable_id,xM:b.x_m,yM:b.y_m,rotation:b.rotation_deg,widthM:b.footprint_width_m,depthM:b.footprint_depth_m,pending:true})),
    ]
    return rows.flatMap(row=>{
      ...
      const widthSvg=Number(row.widthM??def?.footprint.widthM??10)/mapMetrics.widthM*1000
      const depthSvg=Number(row.depthM??def?.footprint.depthM??10)/mapMetrics.heightM*1000
      return[{...row,name:def?.name??row.typeId,mapX:projection.x(geo.lon),mapY:projection.y(geo.lat),widthSvg,depthSvg,visual:getBuildingVisual(row.typeId,'earth')}]
    })
  },[projection,data,spatial,mapMetrics])

  function featureVisible(type:string){
    if(type==='water'||type==='waterway')return layers.water
    if(type==='forest'||type==='farmland')return layers.landuse
    if(type==='road'||type==='rail'||type==='industrial'||type==='public'||type==='building')return layers.infrastructure
    return true
  }
  function toggleLayer(key:LayerKey){setLayers(v=>({...v,[key]:!v[key]}))}

  function pointerToPreview(e:React.PointerEvent<HTMLElement>){
    if(!projection||!data?.region?.origin||!mapGroupRef.current)return null
    const svg=mapGroupRef.current.ownerSVGElement,ctm=mapGroupRef.current.getScreenCTM()
    if(!svg||!ctm)return null
    const point=svg.createSVGPoint();point.x=e.clientX;point.y=e.clientY
    const local=point.matrixTransform(ctm.inverse())
    const geo={lon:projection.lon(local.x),lat:projection.lat(local.y)},metric=geoToLocalMeters(geo,data.region.origin)
    return{mapX:local.x,mapY:local.y,xM:metric.eastM,yM:metric.northM}
  }

  async function placeBuilding(){
    if(!selectedBuilding||!preview||placing)return
    ...
    try{
      ...
      const response=await fetch('/api/game/build/spatial',{method:'POST',headers:{Authorization:`Bearer ${token}`,'Content-Type':'application/json'},body:JSON.stringify({buildableId:selectedBuilding.id,location:'earth',xM:preview.xM,yM:preview.yM})})
      const json=await response.json();if(!response.ok)throw new Error(json.error??'Bauauftrag fehlgeschlagen')
      setBuildMessage(`${selectedBuilding.name}: Bauauftrag angelegt`);await loadSpatial()
  ...
  return <div className="earth-shell">
    <div className="earth-head">
      <div><small>NOXIA EARTH · SAUERLAND 2086</small><h1>Technikstandort Deutschland · Sauerland</h1><p>Reale Geometrie und reale Entfernungen sind Ground Truth. NOXIA-Gebäude verwenden dieselben metrischen Weltkoordinaten.</p></div>
      <div className="earth-actions">
        <button onClick={()=>toggleLayer('sites')}>{layers.sites?'Standorte ausblenden':'Standorte einblenden'}</button>
        <select value={buildTypeId??''} onChange={e=>{setBuildTypeId(e.target.value||null);setBuildMessage(null)}}><option value="">Bauen …</option>{(spatial?.available??[]).map(b=><option key={b.id} value={b.id}>{b.name} · {b.cost} Cr · {b.footprint.widthM}×{b.footprint.depthM} m</option>)}</select>
        {buildTypeId&&<button onClick={()=>{setBuildTypeId(null);setPreview(null);setBuildMessage(null)}}>Abbrechen</button>}
        <div className="earth-stats"><b>{placed.length}</b><span>NOXIA-Bauten</span></div>
      </div>
    </div>

    <div className={`earth-map ${buildTypeId?'building':''}`}
      onWheel={e=>{e.preventDefault();setZoom(z=>Math.min(128,Math.max(.7,z*(e.deltaY<0?1.15:.87))))}}
      ...
      onPointerUp={e=>{if(buildTypeId){void placeBuilding();return}drag.current=null;try{e.currentTarget.releasePointerCapture(e.pointerId)}catch{}}}>
      <svg viewBox="0 0 1000 1000" preserveAspectRatio="xMidYMid slice">
        <rect width="1000" height="1000" fill="#9caf78"/>
        <g ref={mapGroupRef} transform={`translate(${offset.x/zoom} ${offset.y/zoom}) scale(${zoom})`}>
          ...
          {layers.noxia&&placed.map(b=>{
            ...
            const spriteW=Math.max(b.widthSvg*spriteScale,22/zoom)
            const spriteH=Math.max(Math.max(b.depthSvg,b.widthSvg*.72)*spriteScale,18/zoom)
            return <g key={`noxia-${b.id}`} transform={`translate(${b.mapX} ${b.mapY}) rotate(${b.rotation??0})`}>
              <ellipse cx={0} cy={b.depthSvg*.18} rx={Math.max(b.widthSvg*.58,5/zoom)} ry={Math.max(b.depthSvg*.32,2.4/zoom)} fill="#14271e" opacity={b.pending?.16:.24}/>
              <rect x={-b.widthSvg/2} y={-b.depthSvg/2} width={Math.max(b.widthSvg,2/zoom)} height={Math.max(b.depthSvg,2/zoom)} rx={1/zoom} fill={b.pending?'#d9a63d':'#eaf1df'} fillOpacity={b.pending?.18:.07} stroke={b.pending?'#9b741f':'#284f54'} strokeOpacity={b.pending?.9:.55} strokeWidth={1.1/zoom} strokeDasharray={b.pending?`${3/zoom} ${2/zoom}`:undefined}/>
              {visual?.mapAsset
                ? <image href={visual.mapAsset} x={-spriteW/2} y={-spriteH*.72} width={spriteW} height={spriteH} preserveAspectRatio="xMidYMid meet" opacity={b.pending?.58:1}/>
                : <rect x={-b.widthSvg/2} y={-b.depthSvg/2} width={Math.max(b.widthSvg,2/zoom)} height={Math.max(b.depthSvg,2/zoom)} rx={1/zoom} fill={b.pending?'#d9a63d':'#1f5967'} fillOpacity={b.pending?.7:.9}/>
              }
              {zoom>=3&&<text x={0} y={-spriteH*.58-5/zoom} textAnchor="middle" fontSize={9/zoom} fontWeight="800" fill="#17313c" paintOrder="stroke" stroke="#f5f2e8" strokeWidth={2/zoom}>{b.name}{b.pending?' · Bau':''}</text>}
            </g>
          })}

          {layers.sites&&candidates.map(c=><circle key={`raw-${c.lat}-${c.lon}`} cx={c.x} cy={c.y} r={4/zoom} fill="#fff4be" stroke="#8d732e" strokeWidth={1/zoom} opacity=".45"/>)}
          {layers.sites&&shortlist.map(c=><g key={c.shortlistLabel} onClick={e=>{e.stopPropagation();setSelected(c.shortlistLabel)}} style={{cursor:'pointer'}}><circle cx={c.x} cy={c.y} r={15/zoom} fill={c.shortlistRank===1?'#efc34d':'#fff3bd'} stroke="#5d4300" strokeWidth={2.5/zoom}/><text x={c.x} y={c.y+4/zoom} textAnchor="middle" fontSize={11/zoom} fontWeight="900" fill="#493500">{c.shortlistLabel}</text></g>)}

          {selectedBuilding&&preview&&mapMetrics&&(()=>{
            const width=selectedBuilding.footprint.widthM/mapMetrics.widthM*1000
            const depth=selectedBuilding.footprint.depthM/mapMetrics.heightM*1000
            const spriteScale=selectedVisual?.mapScale??1.7
            const spriteW=Math.max(width*spriteScale,22/zoom),spriteH=Math.max(Math.max(depth,width*.72)*spriteScale,18/zoom)
            return <g transform={`translate(${preview.mapX} ${preview.mapY})`} pointerEvents="none">
              <rect x={-width/2} y={-depth/2} width={width} height={depth} fill="#e9c95e" fillOpacity=".32" stroke="#6e5712" strokeWidth={2/zoom} strokeDasharray={`${5/zoom} ${3/zoom}`}/>
              {selectedVisual?.mapAsset&&<image href={selectedVisual.mapAsset} x={-spriteW/2} y={-spriteH*.72} width={spriteW} height={spriteH} preserveAspectRatio="xMidYMid meet" opacity=".58"/>}
              <circle r={3/zoom} fill="#6e5712"/>
            </g>
          })()}
        </g>
      </svg>
      ...
      <div className="earth-help">{buildTypeId?'Bauposition wählen · Esc: abbrechen':'Mausrad: Zoom · Ziehen: Karte · Layer oben rechts'}</div>
    </div>

    <div className="earth-foot"><span>{candidateData?.attribution||data.attribution}</span><span>DEM-Relief: Copernicus/Open-Meteo · Bebaubarkeit aus metrischen Terrain-Zellen</span></div>
    ...
    <style jsx>{`
      ...
      .earth-map-tools{position:absolute;left:16px;top:14px;display:flex;align-items:flex-start;gap:12px;pointer-events:none;color:#17313c;text-shadow:0 1px 2px #fff,0 0 7px #f4f1e6}.earth-compass{width:30px;height:38px;display:grid;place-items:center;position:relative;color:#17313c}.earth-compass span{position:absolute;top:0;font-size:10px;font-weight:900}.earth-compass b{font-size:26px;line-height:1;margin-top:8px}.earth-scale{min-width:72px;padding-top:2px;color:#17313c}.earth-scale span{display:block;font-size:10px;font-weight:900;margin-bottom:3px;text-align:center}.earth-scale i{display:block;height:7px;border-left:2px solid #17313c;border-right:2px solid #17313c;border-bottom:3px solid #17313c;box-sizing:border-box;filter:drop-shadow(0 1px 1px #fff)}
