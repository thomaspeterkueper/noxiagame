...
export default function EarthRegionPreview(){
  ...
  useEffect(()=>{const cancel=(e:KeyboardEvent)=>{if(e.key==='Escape'){if(buildMenuOpen)setBuildMenuOpen(false);else{setSelectedSpot(null);setBuildMessage(null)}}};window.addEventListener('keydown',cancel);return()=>window.removeEventListener('keydown',cancel)},[buildMenuOpen])
  ...
  async function focusGeoPoint(point:GeoPoint,label:string){
    ...
    setFocusLoading(true);setFocusError(null);setSelectedSpot(null);setBuildMenuOpen(false);setBuildMessage(null)
    try{
  ...
  function resetOverview(){if(overviewData)setData(overviewData);setZoom(1);setOffset({x:0,y:0});setFocusLabel(null);setFocusError(null);setSelected(null);setSelectedSpot(null);setBuildMenuOpen(false);setBuildMessage(null)}
  ...
  async function placeBuilding(building:BuildingDef,target:SelectedSpot){if(placing)return;setPlacing(true);setBuildMessage('Bauauftrag wird geprüft …');try{const token=await getToken();if(!token)throw new Error('Nicht angemeldet');const response=await fetch('/api/game/build/spatial',{method:'POST',headers:{Authorization:`Bearer ${token}`,'Content-Type':'application/json'},body:JSON.stringify({buildableId:building.id,location:'earth',xM:target.xM,yM:target.yM})});const json=await response.json();if(!response.ok)throw new Error(json.error??'Bauauftrag fehlgeschlagen');setBuildMessage(`${building.name}: Bauauftrag angelegt`);setBuildMenuOpen(false);setLayers(v=>({...v,noxia:true}));await loadSpatial()}catch(error){setBuildMessage(error instanceof Error?error.message:String(error))}finally{setPlacing(false)}}
  ...
  return <div className="earth-shell">
    ...
    <div className="earth-map"
      ...
      <svg viewBox="0 0 1000 1000" preserveAspectRatio="xMidYMid slice" onClick={e=>{if(suppressMapClick.current){suppressMapClick.current=false;return}const next=pointerToSpot(e);if(next){setSelectedSpot(next);setBuildMenuOpen(false);setBuildMessage(null);setSelected(null)}}}>
        ...
        <g ref={mapGroupRef} transform={`translate(${offset.x} ${offset.y}) scale(${zoom})`}>
          ...
          {selectedSpot&&<g transform={`translate(${selectedSpot.mapX} ${selectedSpot.mapY})`} pointerEvents="none"><circle r={12/zoom} fill="#f5d75f" fillOpacity=".22" stroke="#5c4510" strokeWidth={2/zoom}/><circle r={3/zoom} fill="#5c4510"/><line x1={-17/zoom} y1={0} x2={-7/zoom} y2={0} stroke="#5c4510" strokeWidth={1.5/zoom}/><line x1={7/zoom} y1={0} x2={17/zoom} y2={0} stroke="#5c4510" strokeWidth={1.5/zoom}/><line x1={0} y1={-17/zoom} x2={0} y2={-7/zoom} stroke="#5c4510" strokeWidth={1.5/zoom}/><line x1={0} y1={7/zoom} x2={0} y2={17/zoom} stroke="#5c4510" strokeWidth={1.5/zoom}/></g>}
        </g>
      </svg>
      ...
      {selectedSpot&&<div className="earth-site-panel" onPointerDown={e=>e.stopPropagation()} onClick={e=>e.stopPropagation()}>
        <div className="earth-site-head"><div><small>AUSGEWÄHLTE STELLE</small><strong>{selectedTerrain?terrainLabel(selectedTerrain.state):'Geländedaten werden gesucht'}</strong></div><button onClick={()=>{setSelectedSpot(null);setBuildMenuOpen(false);setBuildMessage(null)}}>×</button></div>
        ...
        {!buildMenuOpen?<button className="earth-build-open" onClick={()=>{setBuildMenuOpen(true);setBuildMessage(null)}}>Bauen</button>:<div className="earth-build-picker"><div className="earth-build-picker-head"><b>Gebäude wählen</b><button onClick={()=>setBuildMenuOpen(false)}>zurück</button></div><div className="earth-build-options">{(spatial?.available??[]).map(building=>{const req=building.requirements,creditsOk=req?.creditsOk??((spatial?.profile?.credits??0)>=building.cost),knowledgeOk=req?.knowledgeOk??true,canBuild=req?.canBuild??(creditsOk&&knowledgeOk);return <button key={building.id} className={`earth-build-option ${canBuild?'':'locked'}`} disabled={!canBuild||placing} onClick={()=>void placeBuilding(building,selectedSpot)}><span className="build-name"><strong>{building.name}</strong><em>{building.cost.toLocaleString('de-DE')} Cr</em></span><span className="build-meta">{building.footprint.widthM}×{building.footprint.depthM} m · {building.buildTimeTicks} Tick{building.buildTimeTicks===1?'':'s'}</span><span className={creditsOk?'req-ok':'req-no'}>{creditsOk?'✓':'×'} Credits</span><span className={knowledgeOk?'req-ok':'req-no'}>{knowledgeOk?'✓':'×'} {req?.requiredLabel?`Wissen: ${req.requiredLabel}`:'keine zusätzliche Wissensvoraussetzung'}</span></button>})}</div></div>}
      ...
      {active&&!selectedSpot&&<div className="earth-candidate"><button onClick={()=>setSelected(null)}>×</button><small>PRÜFSTANDORT {active.shortlistLabel}</small><strong>{active.score.toFixed(0)} / 100</strong><span>{active.elevationM.toFixed(0)} m Höhe · {active.slopePercent.toFixed(1)} % Neigung · {active.reliefM.toFixed(0)} m Relief</span><span>Straße {active.roadDistanceM??'–'} m · Bahn {active.railDistanceM??'–'} m</span><p>{active.reasons.join(' · ')}</p><em>{active.shortlistReason}. Noch keine kanonische Platzierung.</em></div>}
      <div className="earth-help">{selectedSpot?'Stelle gewählt · Gelände prüfen · Bauen':data.detail?'Stelle anklicken · Mausrad: Zoom · Ziehen: Karte':'Stadt/Punkt öffnen oder Stelle direkt untersuchen'}</div>
    </div>
    ...
    <style jsx>{`
      ...
      .earth-site-panel{position:absolute;left:14px;bottom:44px;z-index:6;width:min(390px,calc(100% - 28px));max-height:calc(100% - 78px);overflow:auto;background:#fffaf0f4;border:1px solid #a8893d;border-radius:11px;padding:12px;box-sizing:border-box;box-shadow:0 10px 34px #2b341f40;cursor:default;backdrop-filter:blur(8px)}.earth-site-head{display:flex;justify-content:space-between;gap:12px}.earth-site-head small{display:block;color:#89691b;font-size:9px;font-weight:900;letter-spacing:.12em}.earth-site-head strong{display:block;font-size:15px;margin-top:2px}.earth-site-head button{border:0;background:none;font-size:20px;cursor:pointer}.earth-analysis-level{display:flex;justify-content:space-between;align-items:center;margin:9px 0;padding:7px 8px;background:#edf0e8;border-radius:6px;font-size:10px}.earth-analysis-level span{color:#68777e}.earth-analysis-level b{color:#335360}.earth-site-facts{display:grid;gap:5px}.earth-site-facts>div{display:flex;justify-content:space-between;gap:12px;border-bottom:1px solid #e4ddc8;padding:5px 2px;font-size:10px}.earth-site-facts span{color:#6c7879}.earth-site-facts b{text-align:right;font-weight:800}.earth-site-facts .locked-info b{color:#907f63}.earth-site-facts .unknown-info b{color:#826f55}.earth-build-message{margin-top:9px;padding:7px 8px;background:#f3e5b9;border-radius:6px;font-size:10px;font-weight:800;color:#725516}.earth-build-open{width:100%;margin-top:10px;border:1px solid #8a6b21;background:#c89d35;color:#fffaf0;border-radius:7px;padding:9px;font-weight:900;cursor:pointer}.earth-build-picker{margin-top:10px}.earth-build-picker-head{display:flex;justify-content:space-between;align-items:center;margin-bottom:7px}.earth-build-picker-head button{border:0;background:none;color:#6d5a2a;text-decoration:underline;cursor:pointer}.earth-build-options{display:grid;gap:6px}.earth-build-option{border:1px solid #c5b581;background:#fffdf7;border-radius:7px;padding:8px;text-align:left;display:grid;grid-template-columns:1fr auto;gap:3px 8px;color:#243940;cursor:pointer}.earth-build-option:hover:not(:disabled){border-color:#8a6b21;background:#fff7dc}.earth-build-option.locked{background:#efeee8;color:#777;cursor:not-allowed}.build-name{grid-column:1/-1;display:flex;justify-content:space-between;gap:10px}.build-name strong{font-size:11px}.build-name em{font-style:normal;font-weight:900;color:#9a7622}.build-meta{grid-column:1/-1;font-size:9px;color:#718087}.req-ok,.req-no{font-size:9px;font-weight:800}.req-ok{color:#39704e}.req-no{color:#9a4f45}.earth-terrain-note{display:block;margin-top:9px;color:#857a66;font-size:8px;line-height:1.35}
