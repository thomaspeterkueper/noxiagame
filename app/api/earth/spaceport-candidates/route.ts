...
export async function GET(request:NextRequest){
 try{
  ...
  return NextResponse.json({ok:true,bounds,candidates:ranked.slice(0,8),shortlist,areaAnalysis,recommendedArea:areaAnalysis[0]??null,evaluatedCells:ranked.length,featureCount:features.length,methodology:{terrain:'elevation, slope and local relief',exclusions:['water','waterway','building','settlement','forest'],access:['road','rail'],shortlist:'up to three candidates separated by at least 1.2 km',area:'1 km radius; usable terrain, expansion reserve, corridor span and access',status:'planning heuristic; not canonical placement'},attribution:'Terrain: Copernicus DEM via Open-Meteo · Geography: © OpenStreetMap contributors, ODbL'},{headers:{'Cache-Control':'public, s-maxage=3600, stale-while-revalidate=86400'}})
