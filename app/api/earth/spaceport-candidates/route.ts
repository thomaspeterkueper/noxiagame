...
export async function GET(request: NextRequest) {
  try {
    ...
    const [elevation, features] = await Promise.all([
      openMeteoElevationSource.load(bounds, 500),
      earthSource.load({ bounds, classes: CURRENT_EARTH_BOOTSTRAP_CLASSES }),
    ])
    ...
    console.error('earth spaceport candidates', error)
    return NextResponse.json({ ok: false, error: error instanceof Error ? error.message : 'Candidate analysis unavailable' }, { status: 502 })
  }
}
