...
export async function GET(req: NextRequest) {
  ...
  if (action === 'buy') {
    ...
    if (newCredits < totalCost) {
      return NextResponse.json({ error: 'Unzureichende Credits' }, { status: 400 })
    }

    if (cargoUsed + amount > ship.cargo_max) {
      return NextResponse.json({ error: 'Frachtraum voll' }, { status: 400 })
    }
  ...
  const updatedCargoMap = {
    ...cargoMap,
    [resource]: newCargoAmount,
  }
