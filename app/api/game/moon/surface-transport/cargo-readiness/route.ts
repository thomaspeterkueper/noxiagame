...
function inventoryItems(snapshot: unknown) {
  ...
  return Array.isArray(items) ? items.filter(item => item && typeof item === 'object' && !Array.isArray(item)) as Record<string, unknown>[] : []
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    console.error('moon cargo mass readiness failed:', message)
