...
type DrillRig = { id:DrillRigId; label:string; maxDepthM:number; energyMultiplier:number; durationMultiplier:number }
...
const DRILL_RIGS: Record<DrillRigId,DrillRig> = {
  core_sample:{id:'core_sample',label:'Mobiles Bohrkern-Set',maxDepthM:10,energyMultiplier:1,durationMultiplier:1},
  drill_rig_rotary:{id:'drill_rig_rotary',label:'Rotationsbohranlage',maxDepthM:50,energyMultiplier:.9,durationMultiplier:.85},
  drill_rig_deep:{id:'drill_rig_deep',label:'Tiefbohranlage',maxDepthM:200,energyMultiplier:.85,durationMultiplier:.8},
}

function rigWear(rig:DrillRig,depthM:number){ return Math.max(2,Math.ceil(2+6*(depthM/rig.maxDepthM))) }
function effectivePreset(preset:DepthPreset,rig:DrillRig){
  return {...preset,energyCost:Math.ceil(preset.energyCost*rig.energyMultiplier),durationSeconds:Math.ceil(preset.durationSeconds*rig.durationMultiplier),wearCost:rigWear(rig,preset.depthM)}
}

async function authenticatedUser(req:NextRequest){
  const token=req.headers.get('authorization')?.split(' ')[1]
  if(!token)return null
  const supabase=createServiceClient(); const {data:{user}}=await supabase.auth.getUser(token); return user??null
}
...
async function findRegionFor(supabase:ReturnType<typeof createServiceClient>,lat:number,lon:number){
  ...
  for(const region of regions??[]){const b=region.bounds as {south:number;west:number;north:number;east:number};if(b&&lat>=b.south&&lat<=b.north&&lon>=b.west&&lon<=b.east)return region}
  return null
}
...
async function resolveDueJobs(supabase:ReturnType<typeof createServiceClient>,profileId:string,locationId:string){
  ...
  const {data:due}=await supabase.from('core_sample_jobs').select('*').eq('profile_id',profileId).eq('location_id',locationId).eq('status','running').lte('completes_at',now)
  for(const job of due??[]){
    const origin={lat:Number(job.latitude_deg),lon:Number(job.longitude_deg)},targetDepthM=Number(job.target_depth_m??10)
    ...
    const nearby=(resources??[]).map((r:any)=>({...r,distanceKm:haversineKm(origin,{lat:Number(r.lat),lon:Number(r.lon)}),modeledDepthM:modeledDepthM(String(r.id),String(r.resource_type))}))
...
export async function GET(req:NextRequest){
  const user=await authenticatedUser(req);if(!user)return NextResponse.json({error:'unauthorized'},{status:401})
  const locationSlug=new URL(req.url).searchParams.get('location');if(!locationSlug)return NextResponse.json({error:'location_required'},{status:400})
  ...
  const [{data:jobs},{data:ownedRows},{data:stateRows}]=await Promise.all([
    ...
    supabase.from('player_instruments').select('instrument_id').eq('profile_id',user.id).in('instrument_id',Object.keys(DRILL_RIGS)),
    supabase.from('player_instrument_state').select('instrument_id,condition_percent').eq('profile_id',user.id).in('instrument_id',Object.keys(DRILL_RIGS)),
  ])
  const owned=new Set((ownedRows??[]).map((r:any)=>r.instrument_id));const condition=new Map((stateRows??[]).map((r:any)=>[r.instrument_id,Number(r.condition_percent)]))
  const rigs=Object.values(DRILL_RIGS).map(r=>({...r,owned:owned.has(r.id),conditionPercent:owned.has(r.id)?(condition.get(r.id)??100):null}))
...
export async function POST(req:NextRequest){
  const user=await authenticatedUser(req);if(!user)return NextResponse.json({error:'unauthorized'},{status:401})
  const body=await req.json().catch(()=>({}));const locationSlug=typeof body.location==='string'?body.location:'';const lat=Number(body.lat),lon=Number(body.lon)
  const preset=DEPTH_PRESETS[(typeof body.depthPreset==='string'?body.depthPreset:'shallow') as DepthPresetId]
  const rig=DRILL_RIGS[(typeof body.rigId==='string'?body.rigId:'core_sample') as DrillRigId]
  if(!locationSlug)return NextResponse.json({error:'location_required'},{status:400});if(!preset)return NextResponse.json({error:'invalid_depth_preset'},{status:400});if(!rig)return NextResponse.json({error:'invalid_drill_rig'},{status:400})
  if(preset.depthM>rig.maxDepthM)return NextResponse.json({error:'drill_rig_depth_exceeded'},{status:400})
  if(!Number.isFinite(lat)||!Number.isFinite(lon)||lat<-90||lat>90||lon<-180||lon>180)return NextResponse.json({error:'invalid_coordinates'},{status:400})
  const supabase=createServiceClient();const {data:location}=await supabase.from('locations').select('id,slug').eq('slug',locationSlug).maybeSingle();if(!location)return NextResponse.json({error:'location_not_found'},{status:404})
  if(!await findRegionFor(supabase,lat,lon))return NextResponse.json({error:'no_region_at_sample_position'},{status:400})
  const effective=effectivePreset(preset,rig)
