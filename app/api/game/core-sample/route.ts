...
type CoreLayer = { fromM:number; toM:number; material:string; code:string; origin:'surface'|'modeled'|'glim_context'; note:string; resourceType?:string }
...
const LITHOLOGY_LABEL:Record<string,string>={su:'Lockersedimente',vb:'Basischer Vulkanit',ss:'Sandstein / Sedimentgestein',pb:'Basisches Plutonit',sm:'Gemischte Sedimente',sc:'Karbonatisches Sedimentgestein',va:'Saurer Vulkanit',mt:'Metamorphes Gestein',pa:'Saures Plutonit',vi:'Intermediärer Vulkanit',wb:'Wasser / Feuchtgebiet',py:'Pyroklastisches Material',pi:'Intermediäres Plutonit',ev:'Evaporit',nd:'Nicht bestimmt',ig:'Magmatisches Gestein'}
const MODELED_MATERIALS=['Tonstein','Sandstein','Schluffstein','Kieslage','verwittertes Festgestein','kompaktes Festgestein','klüftiges Festgestein','mineralisierte Ader']
...
async function nearestLithology(supabase:ReturnType<typeof createServiceClient>,lat:number,lon:number){
 const {data}=await supabase.from('geology_lithology_grid').select('lat,lon,lithology_class').gte('lat',lat-.6).lte('lat',lat+.6).gte('lon',lon-.6).lte('lon',lon+.6)
 return (data??[]).map((r:any)=>({...r,d:haversineKm({lat,lon},{lat:Number(r.lat),lon:Number(r.lon)})})).sort((a:any,b:any)=>a.d-b.d)[0]??null
}

function buildCoreLayers(lat:number,lon:number,targetDepthM:number,glimClass:string|null,hit:any|null):CoreLayer[]{
 const key=`${lat.toFixed(5)}:${lon.toFixed(5)}:stratigraphy-v1`
 const layers:CoreLayer[]=[]
 const soil=Math.min(targetDepthM,Number((.4+hashUnit(`${key}:soil`)*1.8).toFixed(1)))
 layers.push({fromM:0,toM:soil,material:'Boden / Verwitterungshorizont',code:'soil',origin:'surface',note:'Oberflächennahe Modellschicht.'})
 let from=soil,idx=0
 while(from<targetDepthM){
  const remaining=targetDepthM-from
  const maxThickness=Math.max(2,Math.min(remaining,targetDepthM<=10?4:targetDepthM<=50?12:35))
  let thickness=Math.max(1.5,1.5+hashUnit(`${key}:thickness:${idx}`)*(maxThickness-1.5))
  thickness=Math.min(remaining,thickness)
  const material=MODELED_MATERIALS[Math.floor(hashUnit(`${key}:material:${idx}`)*MODELED_MATERIALS.length)%MODELED_MATERIALS.length]
  layers.push({fromM:Number(from.toFixed(1)),toM:Number((from+thickness).toFixed(1)),material,code:`modeled_${idx}`,origin:'modeled',note:'Deterministische NOXIA-Schicht, keine reale Bohrdatenlage.'})
  from+=thickness;idx++
 }
 if(glimClass&&layers.length){
  const last=layers[layers.length-1]
  last.material=`${LITHOLOGY_LABEL[glimClass]??glimClass} (Makrokontext)`
  last.code=`glim_${glimClass}`
  last.origin='glim_context'
  last.note='Unterste Modellschicht an die nächstgelegene GLiM-Makrolithologie angelehnt; GLiM beschreibt keine lokale Bohrschicht.'
 }
 if(hit){
  const center=Number(hit.modeledDepthM)
  const half=Math.max(.5,Math.min(3,targetDepthM*.02))
  const mineralFrom=Math.max(0,Number((center-half).toFixed(1))),mineralTo=Math.min(targetDepthM,Number((center+half).toFixed(1)))
  if(mineralTo>mineralFrom)layers.push({fromM:mineralFrom,toM:mineralTo,material:`Mineralisierte Zone: ${hit.resource_type}`,code:'resource_intersection',origin:'modeled',note:'Vom NOXIA-Ressourcenmodell abgeleitete Schnittzone.',resourceType:String(hit.resource_type)})
 }
 return layers.sort((a,b)=>a.fromM-b.fromM||a.toM-b.toM)
}
...
async function resolveDueJobs(supabase:ReturnType<typeof createServiceClient>,profileId:string,locationId:string){
 ...
 for(const job of due??[]){
  ...
  const[{data:resources},glim]=await Promise.all([supabase.from('region_resources').select('id,resource_type,lat,lon,abundance,properties').eq('region_id',region.id),nearestLithology(supabase,origin.lat,origin.lon)])
  ...
  const coreLayers=buildCoreLayers(origin.lat,origin.lon,targetDepthM,glim?.lithology_class??null,hit)
  const common={radius_m:5,target_depth_m:targetDepthM,depth_model:'noxia_modeled_depth_v1',stratigraphy_model:'noxia_stratigraphy_v1',glim_lithology_class:glim?.lithology_class??null,core_layers:coreLayers}
