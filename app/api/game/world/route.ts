...
// Erstellt:     30.05.2026
// Aktualisiert: 20.07.2026 — lease_price + occupant_id in select
// Version:      0.7.0
//
...
export async function GET() {
  ...
  const { data: allEntities } = await supabase
    .from('tile_entities')
    .select('id, profile_id, owner_class, owner_id, actor_id, occupant_id, entity_type, entity_id, tile_level, tile_row, tile_col, location_id, built_at, asking_price, lease_price, profiles(username), locations(id, slug, name)')
