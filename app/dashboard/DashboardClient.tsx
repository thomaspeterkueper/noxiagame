...
import EarthRegionPreview from '@/app/earth/EarthRegionPreview'
...
function DashboardClientInner({ locations: initialLocations, prices, orders: initialOrders }: { locations: any[]; prices: any[]; orders: any[] }) {
  ...
  const handleInteriorAction = (kind: 'market'|'shipyard'|'navigation'|'ship'|'parts'|null) => {
    ...
    if (kind === 'shipyard')   setShipyardOpen(true)
    if (kind === 'navigation') setSolarSystemOpen(true)
    if (kind === 'ship')       setShipInterior(true)
    if (kind === 'parts')      setShipyardOpen(true)   // Ersatzteile: vorerst Werft-Overlay (Modul-Kauf ist dort verankert)
  }
    ...
    <div style={{ minHeight: '100vh', background: T.bg, color: T.ink, fontFamily: 'system-ui, sans-serif', display: 'flex', flexDirection: 'column' }}>
      ...
      {solarSystemOpen && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 2100, background: 'rgba(0,0,0,0.85)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}
          ...
          <div style={{ width: 'min(700px, 95vw)', background: '#070b14', border: '1px solid #1d2a3d', borderRadius: 16, padding: '1rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
              <span style={{ color: '#c9a961', fontFamily: 'monospace', fontSize: '0.75rem', fontWeight: 700 }}>🧭 NAVIGATIONSRAUM — LIVE-PROJEKTION</span>
              <button onClick={() => setSolarSystemOpen(false)} style={{ background: 'none', border: '1px solid #1d2a3d', color: '#5a6b7a', borderRadius: 6, padding: '3px 10px', cursor: 'pointer', fontSize: '0.75rem' }}>ESC ✕</button>
            </div>
            <SolarSystem currentTick={stats?.tickNumber ?? 0} shipRange={shipRange} currentLocation={location} />
          </div>
        </div>
      )}
