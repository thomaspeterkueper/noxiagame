...
function moving(resident: Resident) {
  return !!resident.lastAction?.includes('travel') || ['moving', 'travelling'].includes(resident.activityState)
}
...
function Person({
  ...
  const role = resident ? (assignment(resident, 'work')?.roleCode ?? resident.activityState) : 'Du'
  return (
    <button
      className={`person ${resident && moving(resident) ? 'walking' : 'idle'} ${selected ? 'selected' : ''} ${player ? 'player' : ''}`}
      onClick={event => {
    ...
    >
      ...
      <i className="leg l" />
      <i className="leg r" />
      {(zoom > 1.45 || selected || player) && (
        <span>
          {player ? 'Du' : resident!.displayName.split(' ')[0]}
          <small>{role}</small>
        </span>
      )}
    </button>
...
export default function WalkableColony({
  ...
  const selectedPerson = selection?.kind === 'person'
    ? residents.find(r => r.id === selection.id) ?? null
    : null
  ...
  const residentPositions = useMemo<ResidentPosition[]>(() => residents
    .map(resident => {
      const workAnchor = anchor(buildings.find(b => b.id === assignment(resident, 'work')?.tileEntityId))
      const homeAnchor = anchor(buildings.find(b => b.id === assignment(resident, 'home')?.tileEntityId))
      const base = workAnchor ?? homeAnchor
      if (!base) return null

      let col = base.col
      let row = base.row
      if (moving(resident) && workAnchor && homeAnchor) {
        const phase = (tick * 0.08 + hash(resident.id)) % 2
        const progress = phase <= 1 ? phase : 2 - phase
        const position = positionOnStreetPath(shortestStreetPath(homeAnchor, workAnchor, streets), progress)
        if (position) {
          col = position.col
          row = position.row
        }
      ...
      return { resident, col, row }
    })
    .filter((value): value is ResidentPosition => value !== null), [residents, buildings, streets, tick, anchor])
  ...
  const depth = useMemo(() => [
    ...
    ...residentPositions.map(position => ({ kind: 'p' as const, d: position.col + position.row, position })),
    { kind: 'me' as const, d: fig.col + fig.row },
  ].sort((a, b) => a.d - b.d), [buildings, residentPositions, fig])

  const workers = selectedBuilding
    ? residents.filter(r => r.assignments.some(a => a.type === 'work' && a.tileEntityId === selectedBuilding.id))
    : []
  const homes = selectedBuilding
    ? residents.filter(r => r.assignments.some(a => a.type === 'home' && a.tileEntityId === selectedBuilding.id))
    : []
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: 100, background: '#081019', display: 'grid', gridTemplateRows: '42px minmax(0,1fr) 48px', fontFamily: 'monospace' }}>
      ...
      <style>{`.person{position:absolute;width:22px;height:34px;border:0;background:transparent;padding:0;cursor:pointer}.person .shadow{position:absolute;left:3px;top:27px;width:17px;height:5px;border-radius:50%;background:#0007}.person .head{position:absolute;left:7px;top:1px;width:8px;height:8px;border-radius:50%;background:#d8ad79}.person .body{position:absolute;left:5px;top:9px;width:12px;height:13px;background:#c9942e}.person.player .body{background:#f0c54e}.person .leg{position:absolute;top:21px;width:4px;height:9px;background:#263848}.person .leg.l{left:5px}.person .leg.r{left:12px}.person>span{position:absolute;top:-15px;left:50%;transform:translateX(-50%);white-space:nowrap;background:#0b131ded;color:#eee0c6;padding:2px 4px;font:7px monospace}.person>span small{display:block;color:#9bc7ad}`}</style>

      <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0 12px', background: '#142230', color: '#f1d57a' }}>
        <b>
          NOXIA · {locationName.toUpperCase()}{' '}
          <span style={{ color: '#9eabb8' }}>POP {population.toLocaleString()} · SIM {residents.length} · {district ? 'DISTRIKT' : 'STRATEGIE'}</span>
        </b>
        <div>
      ...
      <main style={{ position: 'relative', overflow: 'hidden' }}>
        <div
          ...
          onPointerMove={event => {
            ...
            if (!current) return
            setViewport(clamp(
              current.vx - (event.clientX - current.x) / zoom,
              current.vy - (event.clientY - current.y) / zoom,
            ))
          }}
          ...
          <div style={{ position: 'absolute', width: CW, height: CH, transformOrigin: '0 0', transform, pointerEvents: 'none' }}>
            {depth.map(item => {
              ...
              if (item.kind === 'p') {
                ...
                return (
                  <div key={item.position.resident.id} style={{ position: 'absolute', left: p.x - 11, top: p.y - 31, pointerEvents: 'auto' }}>
                    <Person
                      resident={item.position.resident}
                      selected={selection?.kind === 'person' && selection.id === item.position.resident.id}
                      zoom={zoom}
                      onClick={() => setSelection({ kind: 'person', id: item.position.resident.id })}
                    />
                  </div>
                )
              }
              ...
              return <div key="player" style={{ position: 'absolute', left: p.x - 11, top: p.y - 31 }}><Person player zoom={zoom} /></div>
            })}
        ...
        {(showPeople || selection) && (
          <aside style={{ ...panel, position: 'absolute', zIndex: 80, top: 10, right: 10, bottom: 10, width: 285, padding: 10, overflowY: 'auto' }}>
            <button onClick={() => { setSelection(null); setShowPeople(false) }} style={{ float: 'right' }}>×</button>
            {showPeople ? (
              residents.map(resident => (
                <button
                  key={resident.id}
                  onClick={() => { setSelection({ kind: 'person', id: resident.id }); setShowPeople(false) }}
                  style={{ display: 'block', width: '100%', padding: 7, marginBottom: 5 }}
                >
              ...
              <>
                ...
                <div>{work?.roleCode ?? selectedPerson.activityState}</div>
                <div style={section}>
                  Arbeit: {workBuilding ? label(workBuilding.entity_id) : '—'}<br />
                  Wohnen: {homeBuilding ? label(homeBuilding.entity_id) : '—'}
                </div>
      ...
      <footer style={{ display: 'flex', justifyContent: 'space-between', padding: '12px', background: '#172230', color: '#9eabb8', fontSize: 9 }}>
        <span>{buildings.length} GEBÄUDE · {streets.length} STRASSEN · {residents.length} PERSONEN · {Math.min(3, streets.length)} ROVER</span>
