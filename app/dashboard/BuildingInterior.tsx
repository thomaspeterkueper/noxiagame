...
// Erstellt:     20.07.2026
// Aktualisiert: 27.08.2026 — Living Population: Personen im Gebäude sichtbar
// Version:      3.2.0
//
...
const LANDING_PAD_ROOMS: RoomDef[] = [
  { id: 'lobby', label: 'Lobby', icon: '🚪', wallHue: 210, detail: 'desk', items: () => ['Ankunftshalle', 'Anzeigetafel mit Abflügen'] },
  { id: 'cargo_office', label: 'Frachtbüro', icon: '📋', wallHue: 40, detail: 'console', action: 'market', actionLabel: '📈 Markt öffnen', items: () => ['Handelsterminal', 'Preisaushang'] },
  { id: 'parts', label: 'Ersatzteil-Handel', icon: '🔧', wallHue: 25, detail: 'shelves', action: 'parts', actionLabel: '🔩 Ersatzteile ansehen', items: () => ['Modul-Regale', 'Werkbank'] },
...
const DOCKING_BAY_ROOMS: RoomDef[] = [
  { id: 'airlock', label: 'Luftschleuse', icon: '🚪', wallHue: 210, detail: 'plain', items: () => ['Druckausgleich bereit'] },
  { id: 'navroom', label: 'Navigationsraum', icon: '🧭', wallHue: 260, detail: 'live_map', items: () => ['Live-Projektion — echte Umlaufbahnen und Distanzen'] },
...
const BUILDING_ROOMS: Record<string, RoomDef[]> = {
  ...
  habitat: [
    ...
    { id: 'quarters', label: 'Wohneinheiten', icon: '🛏', wallHue: 220, detail: 'beds', items: ({ population }) => [`${Math.min(100, Math.ceil(population/10))} von 100 Plätzen belegt`] },
  ],
  mine: [
    { id: 'entry', label: 'Stollen-Eingang', icon: '⛏', wallHue: 30, detail: 'plain', items: () => ['Förderwagen', 'Sicherheitshinweis'] },
    { id: 'shaft', label: 'Förderanlage', icon: '⚙️', wallHue: 25, detail: 'reactor', items: ({ production }) => [`Förderung: ${production ?? 0}t/Tick`, 'Förderband aktiv'] },
    { id: 'storage', label: 'Erzlager', icon: '📦', wallHue: 40, detail: 'crates', items: ({ stock }) => [`Lagerbestand: ${stock ?? 0}t`, 'Bereit zum Abtransport'] },
  ],
  solar: [
    { id: 'control', label: 'Kontrollraum', icon: '☀️', wallHue: 45, detail: 'console', items: ({ production }) => [`Produktion: ${production ?? 0}t Energie/Tick`, 'Paneele optimal ausgerichtet'] },
    { id: 'buffer', label: 'Pufferspeicher', icon: '🔋', wallHue: 200, detail: 'reactor', items: () => ['Akku-Bank', 'Ladezustand nominal'] },
  ],
  bank: [
    { id: 'lobby', label: 'Empfang', icon: '🏦', wallHue: 45, detail: 'desk', items: () => ['Wartebereich', 'Auskunftsschalter'] },
    { id: 'vault', label: 'Tresorraum', icon: '🔐', wallHue: 45, detail: 'crates', items: ({ credits }) => [`Kontostand: ${credits.toLocaleString()} Cr`, 'Zinssatz: 1.2%/Tick'] },
  ],
  school: [
    { id: 'hall', label: 'Hörsaal', icon: '🏫', wallHue: 260, detail: 'desk', items: () => ['Nächste Vorlesung: Orbital-Mechanik'] },
    { id: 'terminal', label: 'SSF-Terminal', icon: '💻', wallHue: 200, detail: 'terminal', items: () => ['Solar Science Foundation — verbunden', 'Lernmodule verfügbar'] },
  ],
  warehouse: [
    { id: 'ramp', label: 'Laderampe', icon: '🚚', wallHue: 40, detail: 'plain', items: () => ['Verladefahrzeug'] },
    { id: 'storage', label: 'Hauptlager', icon: '📦', wallHue: 40, detail: 'shelves', items: ({ stock }) => [`Lagerbestand: ${stock ?? 0}t gesamt`, 'Kapazität: 500t'] },
  ],
  shipyard: [
    { id: 'hall', label: 'Empfangshalle', icon: '⚙️', wallHue: 210, detail: 'desk', items: () => ['Auftragsliste'] },
    { id: 'assembly', label: 'Montagehalle', icon: '🔧', wallHue: 25, detail: 'reactor', items: () => ['Montage-Roboter aktiv', 'Nächstes Schiff: Mk.I'] },
  ],
  admin: [
    { id: 'lobby', label: 'Empfang', icon: '🏛', wallHue: 210, detail: 'desk', items: () => ['Anzeigetafel'] },
    { id: 'office', label: 'Büros', icon: '📋', wallHue: 45, detail: 'terminal', items: ({ population }) => [`Verwaltung für ${population.toLocaleString()} Einwohner`, 'Offene Anträge: 0'] },
  ],
  command_center: [
    { id: 'control', label: 'Kontrollraum', icon: '📡', wallHue: 210, detail: 'console', items: () => ['Alle Systeme nominal', 'Orbitalverbindung aktiv'] },
    { id: 'comm', label: 'Kommunikation', icon: '📻', wallHue: 200, detail: 'terminal', items: () => ['Nächste Kommunikation: Erde +8min'] },
  ],
}
  ...
  return (
    <svg viewBox="0 0 400 220" style={{ width: '100%', display: 'block', background: '#05070c' }}>
      ...
      {room.detail === 'console' && <>
        ...
        {[168,185,202,219].map((x,i) => <circle key={i} cx={x} cy={95} r={2} fill={i % 2 === 0 ? '#4aff7a' : '#c9a961'} opacity={0.8} />)}
      </>}
      ...
      {room.detail === 'beds' && <>{[0,1,2,3].map(i => <rect key={i} x={150+i*26} y={70} width={20} height={38} rx={2} fill="#1a2a3a" stroke="#3a5a7a" strokeWidth={1} />)}</>}
      {room.detail === 'reactor' && <>
        <circle cx={200} cy={85} r={22} fill="none" stroke="#ff8a1a" strokeWidth={2} opacity={0.7} />
        <circle cx={200} cy={85} r={13} fill="#ff8a1a" opacity={0.5}><animate attributeName="opacity" values="0.3;0.7;0.3" dur="1.5s" repeatCount="indefinite" /></circle>
      </>}
      {room.detail === 'desk' && <>
      ...
      {room.detail === 'bar' && <>
        ...
        {[0,1,2].map(i => <rect key={i} x={165+i*22} y={72} width={6} height={16} rx={1} fill="#8a4a30" opacity={0.8} />)}
      </>}
      {room.detail === 'stars' && <>
        <circle cx={200} cy={85} r={28} fill="none" stroke="#4a90d0" strokeWidth={1} opacity={0.4} />
        {[...Array(12)].map((_,i) => { const a=(i/12)*Math.PI*2; const r=8+(i%3)*6; return <circle key={i} cx={200+Math.cos(a)*r} cy={85+Math.sin(a)*r*0.6} r={1} fill="#c9d8ff" opacity={0.7} /> })}
...
export default function BuildingInterior({
  ...
  const ctxBase: RoomCtx = { stock: undefined, production: undefined, isOwn, population, credits, hasShipyard }
  ...
  useEffect(() => {
    const fn = (e: KeyboardEvent) => {
      ...
      if ((e.key === ' ' || e.key === 'Enter') && room.action && onAction) { onAction(room.action); e.preventDefault() }
    }
  ...
  return (
    <div style={{ background: '#000', border: '2px solid #4a3a1a', borderRadius: 4, overflow: 'hidden', boxShadow: '0 4px 20px rgba(0,0,0,0.3)', fontFamily: 'monospace' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '4px 10px', background: '#151510', borderBottom: '1px solid #4a3a1a' }}>
        ...
        <span style={{ color: '#c9d060', fontSize: '0.58rem' }}>RAUM {roomIdx + 1}/{rooms.length}</span>
      </div>
      ...
      <BuildingResidentsCard tileEntityId={entity.id} />

      {room.action && onAction && (
        <div style={{ padding: '0 10px 8px', background: '#0a0a08' }}>
          <button onClick={() => onAction(room.action!)} style={{ width: '100%', padding: '8px', background: '#1a3a1a', border: '1px solid #4a8a4a', borderRadius: 4, color: '#a0e0a0', cursor: 'pointer', fontSize: '0.68rem', fontFamily: 'monospace', fontWeight: 700 }}>{room.actionLabel ?? 'Öffnen'}</button>
        </div>
      )}

      <div style={{ display: 'flex', gap: 4, justifyContent: 'center', padding: '7px', background: '#0a0a08', borderTop: '1px solid #2a2418' }}>
        <button disabled={!canPrev} onClick={() => setRoomIdx(i => i - 1)} style={{ flex: 1, padding: '5px 4px', background: canPrev ? '#1a2a1a' : '#0d0d0a', border: `1px solid ${canPrev ? '#4a6a3a' : '#2a2418'}`, borderRadius: 4, color: canPrev ? '#c9d060' : '#3a3a30', cursor: canPrev ? 'pointer' : 'not-allowed', fontSize: '0.6rem', fontFamily: 'monospace' }}>◀ Zurück</button>
        <button disabled={!canNext} onClick={() => setRoomIdx(i => i + 1)} style={{ flex: 1, padding: '5px 4px', background: canNext ? '#1a2a1a' : '#0d0d0a', border: `1px solid ${canNext ? '#4a6a3a' : '#2a2418'}`, borderRadius: 4, color: canNext ? '#c9d060' : '#3a3a30', cursor: canNext ? 'pointer' : 'not-allowed', fontSize: '0.6rem', fontFamily: 'monospace' }}>Weiter ▶</button>
      </div>
      <button onClick={onClose} style={{ width: '100%', padding: '6px', background: '#151510', border: 'none', borderTop: '1px solid #2a2418', cursor: 'pointer', fontSize: '0.6rem', color: '#8a7a50' }}>✕ Gebäude verlassen (ESC)</button>
    </div>
  )
}
