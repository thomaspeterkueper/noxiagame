...
const chrome=<style>{`
.noxia-primary-colony{position:fixed;z-index:900;left:18px;right:18px;top:84px;bottom:18px;overflow:hidden;background:#081019;border:1px solid #294259;border-radius:12px;box-shadow:0 18px 55px rgba(3,8,13,.46);display:grid;grid-template-rows:46px minmax(0,1fr)}
.noxia-primary-hudrail{position:relative;z-index:170;background:linear-gradient(180deg,#122334,#0b1824);border-bottom:1px solid #294259;overflow:hidden}
.noxia-primary-mapframe{position:relative;min-height:0;overflow:hidden;background:#081019}
...
@media(max-width:900px){.noxia-primary-colony{left:6px;right:6px;top:78px;bottom:6px;border-radius:8px}}
`}</style>

export default function DashboardPrimaryColony(){
 const location=useGameStore(s=>s.location)
 const router=useRouter()
 const mode=useGameModeStore(s=>s.mode),interiorBuildingId=useGameModeStore(s=>s.interiorBuildingId),enterColony=useGameModeStore(s=>s.enterColony),enterInterior=useGameModeStore(s=>s.enterInterior),resetForLocation=useGameModeStore(s=>s.resetForLocation)
 ...
 // Earth no longer exposes a second planning surface. The bright Sauerland
 // isometric renderer is the canonical Earth world; its logical 32x24 cells
 // remain internal placement coordinates only.
 useEffect(()=>{
   if(location==='earth'&&mode!=='interior') router.replace('/dashboard/sauerland-isometric')
 ...
 const simulation=useMemo(()=>simulateColonyTick(Array.isArray(current?.location_resources)?current.location_resources as any[]:[],localEntities,current?.population??0),[current?.location_resources,current?.population,localEntities])
 ...
 if(location==='earth'&&mode!=='interior')return null
 ...
 return <>{chrome}<ColonyHudStyles/><div className="noxia-primary-colony"><div className="noxia-primary-hudrail"><ColonyHudOverlay builds={localBuilds} simulation={simulation}/></div><div className="noxia-primary-mapframe"><WalkableColony locationSlug={location} locationName={current.name??location} population={current.population??0} entities={localEntities as any} pending={localBuilds} residents={residents} ships={[]} locationId={current.id} userId={userId} onClose={()=>{}} onEnterBuilding={b=>enterInterior(b.id)}/><ColonyConversationLayer locationSlug={location} population={current.population??0} entities={localEntities as any} pending={localBuilds} residents={residents} userId={userId}/></div></div></>
}
