...
export default function ScannerFirstPerson({scannerOnline,scanning,scannerLabel,onScan,scannerPosition=null,signals=[],scanRadius=4,scanRadiusKm=.3}:Props){
 ...
 useEffect(()=>{const down=(e:KeyboardEvent)=>{const k=e.key.toLowerCase();if(['w','a','s','d','arrowleft','arrowright','e'].includes(k))e.preventDefault();if(k==='e')interact();keysRef.current.add(k)},up=(e:KeyboardEvent)=>keysRef.current.delete(e.key.toLowerCase());window.addEventListener('keydown',down);window.addEventListener('keyup',up);return()=>{window.removeEventListener('keydown',down);window.removeEventListener('keyup',up)}},[interact])
 useEffect(()=>{const canvas=canvasRef.current;if(!canvas)return;const ctx=canvas.getContext('2d');if(!ctx)return;let raf=0,previous=performance.now()
  const project=(wx:number,wy:number,wz:number,p:Player,w:number,h:number)=>{const dx=wx-p.x,dz=wz-p.z,sin=Math.sin(-p.yaw),cos=Math.cos(-p.yaw),cx=dx*cos-dz*sin,cz=dx*sin+dz*cos;if(cz<=.12)return null;const focal=Math.min(w,h)*.92;return{x:w/2+(cx/cz)*focal,y:h*.53-(wy/cz)*focal,z:cz}}
  const line3d=(a:[number,number,number],b:[number,number,number],p:Player,w:number,h:number,color:string,width=1)=>{const pa=project(...a,p,w,h),pb=project(...b,p,w,h);if(!pa||!pb)return;ctx.beginPath();ctx.moveTo(pa.x,pa.y);ctx.lineTo(pb.x,pb.y);ctx.strokeStyle=color;ctx.lineWidth=width;ctx.stroke()}
  const panel3d=(x0:number,x1:number,y0:number,y1:number,z:number,p:Player,w:number,h:number,fill:string,stroke:string)=>{const pts=[[x0,y0,z],[x1,y0,z],[x1,y1,z],[x0,y1,z]].map(v=>project(v[0],v[1],v[2],p,w,h));if(pts.some(v=>!v))return;ctx.beginPath();ctx.moveTo(pts[0]!.x,pts[0]!.y);for(let i=1;i<4;i++)ctx.lineTo(pts[i]!.x,pts[i]!.y);ctx.closePath();ctx.fillStyle=fill;ctx.fill();ctx.strokeStyle=stroke;ctx.lineWidth=1.5;ctx.stroke()}
  const render=(now:number)=>{const rect=canvas.getBoundingClientRect(),dpr=Math.min(2,window.devicePixelRatio||1),tw=Math.max(1,Math.floor(rect.width*dpr)),th=Math.max(1,Math.floor(rect.height*dpr));if(canvas.width!==tw||canvas.height!==th){canvas.width=tw;canvas.height=th}ctx.setTransform(dpr,0,0,dpr,0,0);const w=rect.width,h=rect.height,dt=Math.min(.05,(now-previous)/1000);previous=now;const p=playerRef.current,k=keysRef.current;p.yaw+=((k.has('arrowleft')?-1:0)+(k.has('arrowright')?1:0))*dt*1.75;const f=(k.has('w')?1:0)-(k.has('s')?1:0),s=(k.has('d')?1:0)-(k.has('a')?1:0);if(f||s){const sin=Math.sin(p.yaw),cos=Math.cos(p.yaw);p.x+=(sin*f+cos*s)*2.6*dt;p.z+=(cos*f-sin*s)*2.6*dt;p.x=clamp(p.x,-ROOM.halfW+.55,ROOM.halfW-.55);p.z=clamp(p.z,-ROOM.halfD+.65,ROOM.halfD-.65)}const close=distance(p.x,p.z,TERMINAL.x,TERMINAL.z)<1.8;setNearTerminal(old=>old===close?old:close)
